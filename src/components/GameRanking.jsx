...
import { getGameRanking, getMyGameVotes, supabase } from "../lib/supabaseClient"

export default function GameRanking({ lobbyId = "global", refreshKey = 0, voterName = "" }) {
  const [games, setGames] = useState([])
  const [myVotes, setMyVotes] = useState({})
  const [showAll, setShowAll] = useState(false)

  async function load() {
    const ranking = await getGameRanking(lobbyId)
    const votes = await getMyGameVotes(lobbyId, voterName)
    const map = {}
    votes.forEach((vote) => {
      map[vote.game_id] = vote.vote
    })
    setMyVotes(map)
    setGames(ranking)
  }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("live-game-votes")
      .on("postgres_changes", { event: "*", schema: "public", table: "game_votes" }, load)
      .subscribe()
    ...
    <section className="mt-12 rounded-[2rem] border border-white/10 bg-white/[0.03] p-4">
      <div className="mb-5 flex items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-bold">Next games</h2>
        </div>
      ...
      {games.length === 0 ? (
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-6 text-center text-neutral-400">
          No votes yet. Swipe a game to start the ranking.
        </div>
      ) : (
        <div className="space-y-3">
          {visibleGames.map((game, index) => (
            <div key={game.gameId} className="flex items-center gap-3 rounded-2xl border border-neutral-800 bg-neutral-900 p-3">
              ...
              {game.poster ? <img src={game.poster} alt={game.title} className="h-16 w-11 rounded-lg object-cover" /> : null}
              <div className="min-w-0 flex-1">
                <h3 className="truncate font-bold">{game.title}</h3>
                <div className="mt-1 flex flex-wrap gap-2 text-xs text-neutral-400">
                  <span>{game.likes} yes</span>
                  <span>•</span>
                  <span>{game.dislikes} skip</span>
                  <span>•</span>
                  <span>Score {game.score}</span>
                  {myVotes[game.gameId] ? (
                    <span className="rounded-full bg-white/10 px-2 py-0.5 text-white">
                      You: {myVotes[game.gameId] === "like" ? "yes" : "skip"}
                    </span>
                  ) : null}
                </div>
              </div>
          ...
          {games.length > 3 ? (
            <button type="button" onClick={() => setShowAll((current) => !current)} className="w-full rounded-2xl border border-white/10 px-4 py-3 font-semibold text-neutral-200 hover:bg-white hover:text-neutral-950">
              {showAll ? "Show top 3 ↑" : `Show all ${games.length} games ↓`}
            </button>
          ) : null}
        </div>
      )}
