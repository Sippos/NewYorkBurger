...
import { addNomination, deleteWatchedByMovieId, getMyVotes, getNominations, getWatched, markWatchedWithRating, resetAllVotes, resetVotesForVoter, setRating, voteMovie } from "../lib/supabaseClient"
...
export default function Movies() {
  ...
  async function handleResetVotes() { if (!activeHandle || !isAdmin) return; const res = await resetVotesForVoter(LOBBY_ID, activeHandle); if (res?.error) { setActionMessage({ type: "error", text: `Could not reset your votes: ${res.error?.message || String(res.error)}` }); return } await loadNominations(); setRankingRefreshKey((current) => current + 1); setActionMessage({ type: "success", text: `Votes reset for ${activeHandle}.` }); setTimeout(() => setActionMessage(null), 2200) }
  async function handleResetAllVotes() { if (!isAdmin) return; const res = await resetAllVotes(LOBBY_ID); if (res?.error) { setActionMessage({ type: "error", text: `Could not reset all votes: ${res.error?.message || String(res.error)}` }); return } await loadNominations(); setRankingRefreshKey((current) => current + 1); setActionMessage({ type: "success", text: "All votes were reset." }); setTimeout(() => setActionMessage(null), 2200) }
  ...
  async function handleSearch(e) { e.preventDefault(); if (!query.trim()) return; if (!canUseApp) { setActionMessage({ type: "error", text: "Create an account with the + button in the navbar first." }); return } setResults(await searchMovies(apiKey, query)) }
  async function handleAddMovie(movie) { if (!canUseApp) { setActionMessage({ type: "error", text: "Create an account with the + button in the navbar first." }); return } const res = await addNomination(movie, activeHandle, LOBBY_ID); if (res?.error) setActionMessage({ type: "error", text: `Could not add movie: ${String(res.error.message || res.error)}` }); else { setActionMessage({ type: "success", text: `"${movie.title}" added to the swipe pile.` }); await loadNominations(); clearSearch() } setTimeout(() => setActionMessage(null), 2500) }
  ...
  return (
    <div className="min-h-screen bg-neutral-950 px-3 py-3 text-white sm:px-4 md:px-6"><div className="mx-auto max-w-5xl"><PageNav active="movies" />
      <section className="mb-5 rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-4 shadow-2xl shadow-black/20 sm:rounded-[1.75rem] md:p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><p className="text-xs uppercase tracking-[0.3em] text-neutral-500">Movie night</p><h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl md:text-4xl">Pick what to watch</h1><p className="mt-3 max-w-2xl text-neutral-400">Search movies, add them to the pile, and vote with your navbar account handle.</p>{!canUseApp ? <p className="mt-3 rounded-2xl border border-yellow-400/30 bg-yellow-400/10 p-3 text-sm text-yellow-200">Create an account with the + button in the navbar before voting or adding movies.</p> : null}</div>{isAdmin ? <div className="flex flex-wrap items-center gap-2 rounded-full border border-white/10 bg-neutral-950/70 px-2 py-2 sm:px-3"><button type="button" className="rounded-full px-3 py-1.5 text-sm text-neutral-400 transition hover:bg-white/10 hover:text-white" onClick={handleResetVotes}>Reset mine</button><button type="button" className="rounded-full px-3 py-1.5 text-sm text-red-300 transition hover:bg-red-400 hover:text-neutral-950" onClick={handleResetAllVotes}>Reset all</button></div> : null}</div><form onSubmit={handleSearch} className="mt-4"><div className="flex gap-2"><input className="min-w-0 flex-1 rounded-2xl border border-white/10 bg-neutral-900 px-4 py-3 outline-none transition focus:border-white/30" placeholder="Search a movie..." value={query} onChange={(e) => setQuery(e.target.value)} />{hasResults ? <button type="button" className="rounded-2xl border border-white/10 px-4 py-3 font-semibold text-neutral-200 transition hover:bg-white hover:text-neutral-950" onClick={clearSearch}>Back</button> : null}<button className="rounded-2xl bg-white px-4 py-3 font-semibold text-neutral-950 transition hover:bg-neutral-200 sm:px-5">Search</button></div></form></section>
