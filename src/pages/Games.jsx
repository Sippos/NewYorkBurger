          ...
          {played.length === 0 ? <p className="text-neutral-400">No played games yet.</p> : (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {played.map((game) => {
                ...
                return (
                  <div key={game.id} className="rounded-3xl border border-white/10 bg-neutral-950/70 p-3 transition hover:border-white/20">
                    <div className="flex gap-3">
                      {game.poster ? (
                        <button type="button" onClick={() => openPlayedGameInfo(game)} className="group relative h-28 w-20 shrink-0 overflow-hidden rounded-2xl text-left" title={`Open ${game.title} details`}>
                          <img src={game.poster} alt="" className="h-full w-full object-cover transition group-hover:scale-105" />
                          <span className="absolute inset-x-1 bottom-1 rounded-full bg-black/70 px-2 py-1 text-center text-[0.6rem] font-semibold text-white opacity-90 backdrop-blur">Details</span>
                        </button>
                      ) : (
                        <button type="button" onClick={() => openPlayedGameInfo(game)} className="flex h-28 w-20 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-neutral-900 px-2 text-center text-xs font-semibold text-neutral-400 hover:bg-white hover:text-neutral-950">
                          Details
                        </button>
                      )}
                      <div className="min-w-0 flex-1">
                        ...
                        <button type="button" onClick={() => openPlayedGameInfo(game)} className="mt-3 inline-flex items-center gap-1.5 rounded-xl border border-white/10 px-3 py-2 text-sm font-semibold text-neutral-200 transition hover:bg-white hover:text-neutral-950">
