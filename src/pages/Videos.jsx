...
export default function Videos() {
  ...
  const shareFallbackUrl = `${import.meta.env.BASE_URL}?share-target=videos`
  ...
  return (
    <div className="min-h-screen bg-neutral-950 px-3 py-3 text-white sm:px-4 md:px-6"><div className="mx-auto max-w-5xl"><PageNav active="videos" />
      <section className="mb-5 rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-4 shadow-2xl shadow-black/20 sm:rounded-[1.75rem] md:p-5"><div><p className="text-xs uppercase tracking-[0.3em] text-neutral-500">Shared link dump</p><h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl md:text-4xl">Upload funny links</h1><p className="mt-3 max-w-2xl text-neutral-400">Paste YouTube, TikTok, or Instagram links. Your navbar account handle is used automatically.</p><p className="mt-2 max-w-2xl text-sm text-neutral-500">iPhone share sheets may not show installed PWAs. Use the iPhone save page after copying a link, or connect it to an iOS Shortcut.</p>{!canUpload ? <p className="mt-3 rounded-2xl border border-yellow-400/30 bg-yellow-400/10 p-3 text-sm text-yellow-200">Create an account with the + button in the navbar before uploading.</p> : null}</div>
        <div className="mt-5 space-y-3"><input className="w-full rounded-2xl border border-white/10 bg-neutral-900 px-4 py-3 outline-none" placeholder="Paste YouTube / TikTok / Instagram link" value={url} onChange={(e) => setUrl(e.target.value)} /><input className="w-full rounded-2xl border border-white/10 bg-neutral-900 px-4 py-3 outline-none" placeholder="Funny title (optional)" value={title} onChange={(e) => setTitle(e.target.value)} /><div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><button type="button" onClick={() => uploadVideo(false)} className="rounded-2xl bg-white px-5 py-3 font-semibold text-neutral-950 transition hover:bg-neutral-200">Upload to feed</button><button type="button" onClick={() => uploadVideo(true)} className="rounded-2xl border border-white/10 px-5 py-3 font-semibold text-white transition hover:bg-white hover:text-neutral-950">Upload as classic</button><a href={shareFallbackUrl} className="rounded-2xl border border-white/10 px-5 py-3 text-center font-semibold text-white transition hover:bg-white hover:text-neutral-950">iPhone save page</a></div></div></section>
