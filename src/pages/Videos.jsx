...
export default function Videos() {
  ...
  async function loadVideos() {
    ...
    if (res?.error) {
      setMessage({ type: "error", text: `Could not load videos: ${res.error?.message || res.error}` })
    } else {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("video-links-live")
      .on("postgres_changes", { event: "*", schema: "public", table: "video_links" }, loadVideos)
      .subscribe()
  ...
  function saveHandle() {
    const clean = cleanHandle(draftHandle)
    if (clean.length < 2) {
      setMessage({ type: "error", text: "Choose an uploader name with at least 2 characters." })
      return
    }
  ...
  function buildVideo() {
    ...
    if (!video) {
      setMessage({ type: "error", text: "Paste a valid YouTube, TikTok, or Instagram link." })
      return null
    }
    if (!canUpload) {
      setMessage({ type: "error", text: "Choose your uploader name first." })
  ...
  async function markClassic(video) {
    const res = await setVideoClassic(video.id, true)
    if (res?.error) {
      setMessage({ type: "error", text: "Could not save classic." })
      return
    }
    await loadVideos()
  }
  ...
  async function saveVideoSettings() {
    ...
    const res = await updateVideoLink(editingVideo.id, { title: editingVideo.title })
    if (res?.error) {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 px-3 py-3 text-white sm:px-4 md:px-6">
      <div className="mx-auto max-w-5xl">
        <PageNav title="Video Links" active="videos" />
        ...
        <section className="mb-5 rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-4 shadow-2xl shadow-black/20 sm:rounded-[1.75rem] md:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-neutral-500">Shared link dump</p>
              <h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl md:text-4xl">Upload funny links</h1>
              <p className="mt-3 max-w-2xl text-neutral-400">Paste YouTube, TikTok, or Instagram links. Every upload shows who posted it, and classics stay pinned in the hall of fame.</p>
            </div>

            {isChoosingHandle ? (
              <div className="w-full rounded-3xl border border-white/10 bg-neutral-950/70 p-3 lg:max-w-sm">
                <div className="text-xs uppercase tracking-[0.25em] text-neutral-500">Uploader</div>
                <div className="mt-2 flex gap-2">
                  <input className="min-w-0 flex-1 rounded-2xl border border-white/10 bg-neutral-900 px-4 py-3 outline-none" placeholder="Your name" value={draftHandle} onChange={(e) => setDraftHandle(e.target.value)} />
                  <button type="button" onClick={saveHandle} className="rounded-2xl bg-white px-4 py-3 font-semibold text-neutral-950">Save</button>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2 rounded-full border border-white/10 bg-neutral-950/70 px-2 py-2 sm:px-3">
                <span className="rounded-full bg-white px-3 py-1.5 text-sm font-semibold text-neutral-950">{handle}</span>
                <button type="button" onClick={() => setIsChoosingHandle(true)} className="rounded-full px-3 py-1.5 text-sm text-neutral-300 hover:bg-white/10 hover:text-white">Switch</button>
              </div>
            )}
          </div>
          ...
          <div className="mt-5 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <button type="button" disabled={!canUpload} onClick={() => uploadVideo(true)} className="rounded-2xl border border-white/10 px-5 py-3 font-semibold text-white transition hover:bg-white hover:text-neutral-950 disabled:opacity-40">Upload as classic</button>
            </div>
          </div>
        </section>
        ...
        <section className="mb-10 rounded-[2rem] border border-white/10 bg-white/[0.03] p-4">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-3xl font-semibold">Video feed</h2>
            </div>
            <div className="text-sm text-neutral-500">{videos.length} uploaded link{videos.length === 1 ? "" : "s"}</div>
          </div>

          {videos.length === 0 ? <p className="text-neutral-400">No links uploaded yet.</p> : (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {videos.map((video) => <VideoCard key={video.id} video={video} onEdit={setEditingVideo} onMarkClassic={markClassic} />)}
            </div>
          )}
        </section>
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-4">
          ...
          {classics.length === 0 ? <p className="text-neutral-400">No classics yet.</p> : (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {classics.map((video) => <VideoCard key={video.id} video={video} onEdit={setEditingVideo} onMarkClassic={markClassic} />)}
            </div>
          )}
        </section>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-neutral-950 p-5 shadow-2xl shadow-black/40">
              ...
              <div className="mt-5 space-y-3">
                ...
                <button type="button" onClick={saveVideoSettings} className="w-full rounded-2xl bg-white px-5 py-3 font-semibold text-black">Save changes</button>
                <button type="button" onClick={() => removeVideo(editingVideo.id)} className="w-full rounded-2xl bg-red-600 px-5 py-3 font-semibold text-white">Delete video</button>
              </div>
            </div>
          </div>
        ) : null}
